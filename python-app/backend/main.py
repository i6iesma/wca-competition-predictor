...
def get_all_users(competition_id,  cursor):
    ...
    for user_id in users_ids:
        ...
        users.append(user)
    return users
def get_pbs(users, event, format, cursor):
    new_users = []
    ...
    debug_user_status(new_users, "get_pbs()")
    return new_users
...
def sort_users(users):
    ...
    debug_user_status(users, "sort_users()")
    return users
...
def fix_centiseconds(users, event):
    ...
    debug_user_status(users, "fix_centiseconds()")
    return users
...
def main(competition_id, event, format, mode):
    ...
    users = get_all_users(competition_id, cursor)
    if mode == "pb":
        
        users = get_pbs(users, event, format, cursor)

    users = sort_users(users)
    users = fix_centiseconds(users, event)
    # Close the connection
    connection.close()
    debug_user_status(users, "main()")
    return users


print(main("LazarilloOpen2023", "333mbf", "single", "pb"))
